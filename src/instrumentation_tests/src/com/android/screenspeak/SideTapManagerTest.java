 ...
 */
public class SideTapManagerTest extends ScreenSpeakInstrumentationTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        mSideTapManager = new SideTapManager(mScreenSpeak, mGestureController);
        mMockIntegratedDetector = new MockIntegratedTapDetector(
                (SensorManager) mScreenSpeak.getSystemService(Context.SENSOR_SERVICE));
    ...
    private void setSingleTapPreference(int stringId) {
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(mScreenSpeak);
        settings.edit().putString(mScreenSpeak.getString(R.string.pref_shortcut_single_tap_key),
                mScreenSpeak.getString(stringId)).commit();
    }

    private void setDoubleTapPreference(int stringId) {
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(mScreenSpeak);
        settings.edit().putString(mScreenSpeak.getString(R.string.pref_shortcut_double_tap_key),
                mScreenSpeak.getString(stringId)).commit();
    }

    private void setTapSensitivityPreference(int stringId) {
        SharedPreferences settings = PreferenceManager.getDefaultSharedPreferences(mScreenSpeak);
        settings.edit().putString(mScreenSpeak.getString(R.string.pref_tap_sensitivity_key),
                mScreenSpeak.getString(stringId)).commit();
    }
    ...
    @MediumTest
    public void testSingleTaps_tapSentToScreenSpeak() {
        ...
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mScreenSpeak);
        String expectedAction = prefs.getString(
                mScreenSpeak.getString(R.string.pref_shortcut_single_tap_key),
                mScreenSpeak.getString(R.string.pref_shortcut_single_tap_default));
        assertEquals(expectedAction, mGestureController.mLastAction);

        /* Tap should not be sent along if service is not active */
        mScreenSpeak.suspendScreenSpeak();
        waitForAccessibilityIdleSync();
        mSideTapManager.onSingleTap(startTime + 4000 * 1000 * 1000L);
        assertEquals(1, mGestureController.mNumPerformCustomGestureCalls);

        /* Make sure it works again after resume */
        mScreenSpeak.resumeScreenSpeak();
        waitForAccessibilityIdleSync();
    ...
    @MediumTest
    public void testDoubleTaps_tapSentToScreenSpeak() {
        ...
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(mScreenSpeak);
        String expectedAction = prefs.getString(
                mScreenSpeak.getString(R.string.pref_shortcut_double_tap_key),
                mScreenSpeak.getString(R.string.pref_shortcut_double_tap_default));
        assertEquals(expectedAction, mGestureController.mLastAction);
    ...
    public void testScreenTurnedOffAndOn_tapDetectorShouldStopAndStart() {
        ...
        mSideTapManager.onReceive(mScreenSpeak, screenOffIntent);
        ...
        mSideTapManager.onReceive(mScreenSpeak, screenOnIntent);
        assertEquals(1, mMockIntegratedDetector.stopCount - stopCountBefore);
...
/*
 * TODO(PW) I'd prefer to use mockito here, but I get confusing
