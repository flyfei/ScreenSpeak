...
import com.android.screenspeak.CursorGranularity;
...
import com.android.screenspeak.volumebutton.VolumeButtonPatternDetector;
...
public class ProcessorVolumeStream implements AccessibilityEventListener,
        ScreenSpeakService.KeyEventListener, VolumeButtonPatternDetector.OnPatternMatchListener {
    ...
    private ScreenSpeakService mService;
    ...
    private void handleBothVolumeKeysLongPressed() {
        if (ScreenSpeakService.isServiceActive() && switchScreenSpeakActiveStateEnabled()) {
            mService.requestSuspendScreenSpeak();
        } else {
            mService.resumeScreenSpeak();
        }
    }

    private boolean switchScreenSpeakActiveStateEnabled() {
    ...
    private void resetDimScreenSettings() {
        ...
        SharedPreferencesUtils.putBooleanPref(prefs, mService.getResources(),
                R.string.pref_dim_when_screenspeak_enabled_key, false);
    }

    private void handleSingleTap(int button) {
        if (ScreenSpeakService.isServiceActive() && attemptNavigation(button)) {
            return;
        }
