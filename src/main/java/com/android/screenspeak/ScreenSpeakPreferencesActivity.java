...
import com.android.screenspeak.labeling.LabelManagerSummaryActivity;
...
/**
 * Activity used to set ScreenSpeak's service preferences.
 */
@SuppressWarnings("deprecation")
public class ScreenSpeakPreferencesActivity extends PreferenceActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        final CheckBoxPreference prefDimEnabled = (CheckBoxPreference) findPreferenceByResId(
                R.string.pref_dim_when_screenspeak_enabled_key);
        ...
        mPrefs.registerOnSharedPreferenceChangeListener(
            new SharedPreferences.OnSharedPreferenceChangeListener() {
                ...
                public void onSharedPreferenceChanged(SharedPreferences sharedPreferences,
                                                      String key) {
                    String dimKey = getString(R.string.pref_dim_when_screenspeak_enabled_key);
                    if (key != null &&  key.equals(dimKey)) {
                        boolean value = mPrefs.getBoolean(dimKey,
                                getResources().getBoolean(
                                        R.bool.pref_dim_when_screenspeak_enabled_default));
        ...
        checkInstalledBacks();
        showScreenSpeakVersion();
        updateScreenSpeakShortcutStatus();

        assignWebIntentToPreference(R.string.pref_play_store_key,
                "https://play.google.com/store/apps/details?id=com.google.android.marvin.screenspeak");
    ...
    private void checkTouchExplorationSupportInner(PreferenceGroup category) {
        ...
        final Intent shortcutsIntent = new Intent(this, ScreenSpeakShortcutPreferencesActivity.class);
        shortcutsScreen.setIntent(shortcutsIntent);
    }

    private void updateScreenSpeakShortcutStatus() {
        ...
        if (Build.VERSION.SDK_INT >= ProcessorVolumeStream.MIN_API_LEVEL) {
            preference.setEnabled(ScreenSpeakService.getInstance() != null || preference.isChecked());
        } else {
            final PreferenceGroup category = (PreferenceGroup) findPreferenceByResId(
                    R.string.pref_category_miscellaneous_key);
            if (category == null) {
    ...
    private void updateDimingPreferenceStatus() {
        ...
        dimPreference.setEnabled(ScreenSpeakService.isServiceActive() || dimPreference.isChecked());
        dimPreference.setOnPreferenceChangeListener(new OnPreferenceChangeListener() {
            ...
            public boolean onPreferenceChange(Preference preference, Object newValue) {
                ...
                if (!boolValue && !ScreenSpeakService.isServiceActive()) {
    ...
    private void checkWebScriptsSupport() {
        // ScreenSpeak can control web script injection on API 18+ only.
        final PreferenceGroup category = (PreferenceGroup) findPreferenceByResId(
    ...
    private void checkInstalledBacks() {
        ...
        final int kickBackVersionCode = PackageManagerUtils.getVersionCode(
                this, ScreenSpeakUpdateHelper.KICKBACK_PACKAGE);
        final boolean removeKickBack = (kickBackVersionCode
                >= ScreenSpeakUpdateHelper.KICKBACK_REQUIRED_VERSION);
        ...
        final int soundBackVersionCode = PackageManagerUtils.getVersionCode(
                this, ScreenSpeakUpdateHelper.SOUNDBACK_PACKAGE);
        final boolean removeSoundBack = (soundBackVersionCode
                >= ScreenSpeakUpdateHelper.SOUNDBACK_REQUIRED_VERSION);
    ...
    /**
     * Shows ScreenSpeak's abbreviated version number in the action bar, and the
     * full version number in the Play Store button.
     */
    private void showScreenSpeakVersion() {
        try {
            ...
            if (actionBar != null) {
                actionBar.setSubtitle(
                        getString(R.string.screenspeak_preferences_subtitle,
    ...
    /**
     * Updates the preference that controls whether ScreenSpeak will attempt to
    ...
    private boolean setTouchExplorationRequested(boolean requestedState) {
        final SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(
                ScreenSpeakPreferencesActivity.this);

        // Update the "requested" state. This will trigger a listener in
        // ScreenSpeak that changes the "actual" state.
        SharedPreferencesUtils.putBooleanPref(prefs, getResources(),
                R.string.pref_explore_by_touch_key, requestedState);

        // If ScreenSpeak is inactive, we should immediately reflect the change in
        // "requested" state.
        if (!ScreenSpeakService.isServiceActive()) {
            return true;
        }
        if(requestedState && ScreenSpeakService.getInstance() != null) {
            ScreenSpeakService.getInstance().showTutorial();
        }
        ...
        LogUtils.log(this, Log.DEBUG, "ScreenSpeak active, waiting for EBT request to take effect");
        return false;
    }
    ...
    private final OnPreferenceChangeListener mPreferenceChangeListener =
            new OnPreferenceChangeListener() {
                ...
                public boolean onPreferenceChange(Preference preference, Object newValue) {
                    ...
                    if (getString(R.string.pref_resume_screenspeak_key).equals(key)) {
                        final String oldValue = SharedPreferencesUtils.getStringPref(
                                mPrefs, getResources(), R.string.pref_resume_screenspeak_key,
                                R.string.pref_resume_screenspeak_default);
