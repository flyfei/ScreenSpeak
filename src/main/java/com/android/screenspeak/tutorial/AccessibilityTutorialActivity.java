...
package com.android.screenspeak.tutorial;
...
public class AccessibilityTutorialActivity extends Activity {
    ...
    protected void onResume() {
        ...
        /*
         * Handle the cases where the tutorial was started with ScreenSpeak in an
         * invalid state (inactive, suspended, or without Explore by Touch
         * enabled).
         */
        final int serviceState = ScreenSpeakService.getServiceState();
        /*
         * Check for suspended state first because touch exploration reports it
         * is disabled when ScreenSpeak is suspended.
         */
        if (serviceState == ScreenSpeakService.SERVICE_STATE_SUSPENDED) {
            ...
            return;
        } else if ((serviceState == ScreenSpeakService.SERVICE_STATE_INACTIVE)
    ...
    private final ScreenSpeakService.ServiceStateListener mServiceStateListener =
            new ScreenSpeakService.ServiceStateListener() {
        @Override
        public void onServiceStateChanged(int newState) {
            if (newState == ScreenSpeakService.SERVICE_STATE_INACTIVE) {
                // If the service dies while the tutorial is active, exit.
                finish();
            } else if (newState == ScreenSpeakService.SERVICE_STATE_SUSPENDED) {
                stopRepeating();
