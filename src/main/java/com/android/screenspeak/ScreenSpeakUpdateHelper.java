...
public class ScreenSpeakUpdateHelper {
    ...
    public ScreenSpeakUpdateHelper(ScreenSpeakService service) {
        mService = service;
    ...
    public void checkUpdate() {
        ...
        // Revision 84 combines the ScreenSpeak and continuous reading breakout
    ...
    /**
     * When a user updates ScreenSpeak on a pre-MR0 release of Jelly Bean that had
     ...
     * <li>Did we just trigger the bug? => Upgrading from the factory ROM version of ScreenSpeak
     * <li>Was explore by touch previously enabled? => Has already run the ScreenSpeak tutorial
     * </ol>
     */
    private boolean needsExploreByTouchHelper(int previousVersion) {
        return (Build.VERSION.SDK_INT == 16) && (previousVersion == 68)
                && !mSharedPreferences.getBoolean(ScreenSpeakService.PREF_FIRST_TIME_USER, true);
    }
    ...
    private void notifyUserOfBuiltInGestureChanges() {
        ...
        notificationIntent.putExtra(NotificationActivity.EXTRA_INT_DIALOG_TITLE,
                R.string.notification_title_screenspeak_gestures_changed);
        notificationIntent.putExtra(NotificationActivity.EXTRA_INT_DIALOG_MESSAGE,
                R.string.screenspeak_built_in_gesture_change_details);
    ...
    private Notification buildGestureChangeNotification(Intent clickIntent) {
        ...
        final String contentText = mService.getString(
                R.string.notification_message_screenspeak_gestures_changed);
    ...
    /**
     * Event listener that helps handle a bug when upgrading ScreenSpeak from the
    ...
    private static class ExploreByTouchUpdateHelper implements AccessibilityEventListener {
        ...
        public ExploreByTouchUpdateHelper(ScreenSpeakService service) {
            mService = service;
        }
