...
package com.android.screenspeak.controller;

import android.annotation.TargetApi;
import com.android.screenspeak.EarconsPlayTask;
...
public class FeedbackControllerApp implements FeedbackController {
    ...
    private void updateHapticFromPreference(SharedPreferences prefs) {
        ...
        if (mUseCompatKickBack) {
            enabled = SecureSettingsUtils.isAccessibilityServiceEnabled(
                    mContext, ScreenSpeakUpdateHelper.KICKBACK_PACKAGE);
        } else {
    ...
    private void updateAuditoryFromPreference(SharedPreferences prefs) {
        ...
        if (mUseCompatSoundBack) {
            enabled = SecureSettingsUtils.isAccessibilityServiceEnabled(
                    mContext, ScreenSpeakUpdateHelper.SOUNDBACK_PACKAGE);
        } else {
            enabled = SharedPreferencesUtils.getBooleanPref(
    ...
    private boolean shouldUseCompatKickBack() {
        if (!PackageManagerUtils.hasPackage(mContext, ScreenSpeakUpdateHelper.KICKBACK_PACKAGE)) {
            return false;
        }

        final int kickBackVersionCode = PackageManagerUtils.getVersionCode(
                mContext, ScreenSpeakUpdateHelper.KICKBACK_PACKAGE);
        return kickBackVersionCode >= ScreenSpeakUpdateHelper.KICKBACK_REQUIRED_VERSION;
    }

    private boolean shouldUseCompatSoundBack() {
        if (!PackageManagerUtils.hasPackage(mContext, ScreenSpeakUpdateHelper.SOUNDBACK_PACKAGE)) {
            return false;
        }

        final int kickBackVersionCode = PackageManagerUtils.getVersionCode(
                mContext, ScreenSpeakUpdateHelper.SOUNDBACK_PACKAGE);
        return kickBackVersionCode >= ScreenSpeakUpdateHelper.SOUNDBACK_REQUIRED_VERSION;
    }
}
